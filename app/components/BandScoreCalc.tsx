...
const SECTIONS = ["Fluency & Coherence", "Lexical Resource", "Grammatical Range", "Pronunciation"] as const;
...
function estimateLevel(band: number) {
  if (band >= 8) return "Expert Speaker";
  if (band >= 7) return "Good Speaker";
  if (band >= 6) return "Competent Speaker";
  if (band >= 5) return "Modest Speaker";
  if (band >= 4) return "Limited Speaker";
  return "Beginner";
}
...
export default function BandScoreCalc() {
  ...
  const level = estimateLevel(overall);
      ...
      <div className="mx-auto max-w-4xl px-6">
        <div className="anim-in mb-16 text-center">
          ...
          <h2 className="text-4xl font-bold leading-tight tracking-tight text-theme-text md:text-5xl">
            Band Score <span className="text-theme-accent">Calculator</span>
          </h2>
          <p className="mx-auto mt-4 max-w-lg text-theme-text-muted">
            Rate yourself across the four IELTS Speaking criteria to estimate your overall band.
          </p>
        </div>
            ...
            {SECTIONS.map((section, i) => (
              <div key={section}>
                <div className="mb-1.5 flex items-center justify-between text-sm">
                  <span className="font-medium text-theme-text">{section}</span>
